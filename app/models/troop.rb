...
class Troop
  ...
  def total
    self.units_hash.values.inject(&:+) || 0
  end

  def validate
    if (self.units_hash.values.select{|a| a < 0}.size > 0)
  ...
  def contains other 
    result = self - other
    result.units_hash.values.select{|a| a < 0}.empty?
  end

  def slow_unit
    self.units_hash.select{|unit,qte| qte > 0}.keys.map{|a| Unit.get(a) }.sort{|a,b| a.speed <=> b.speed }.first
  end
  ...
  def win?(moral,wall,night_bonus)
    ...
    self.units_hash.each do |unit, qte|
  ...
  def upgrade(disponible,pillage)
    ...
    disponible = disponible.clone.units_hash
    ...
    raise ImpossibleUpgrade.new if (self.units_hash == troops)
  ...
  def population
    troops = self.units_hash
    total = 0
    self.units_hash.map do |unit,qte|
  ...
  def increase_population(disponible,target_population)
    result = self.clone.units_hash
    disponible = (disponible - self).units_hash
  ...
  def +(other)
    result = self.units_hash.clone
    other = other.units_hash.clone
