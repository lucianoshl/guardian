...
RSpec.describe Task::PillageAround, type: :model do
  it "pillage_test_local" do 
    ...
    # Dummy.new.run
    Job::SendAttack.new(event_time: Time.zone.now + 9.minutes + 30.seconds,troop: Troop.new(spy: 5),coordinate: '398|413',origin: Village.where(x:399,y:413).first).execute
  end
